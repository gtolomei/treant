...
CMD_NAME="${PYTHON_SCRIPT_NAME} $DATASET_NAME $TRAIN_SET_FILE_PATH $VALID_SET_FILE_PATH $TEST_SET_FILE_PATH $LEARNING_ALGORITHM $N_ESTIMATORS -l sse -o $MODELS_DIR -n $N_INSTANCES -d $MAX_DEPTH -i $N_INSTANCES_PER_NODE -b $ATTACKER_BUDGET -a $ATTACKS_FILE -r $ATTACK_RULES_FILE -bf $FEATURE_SAMPLING"
