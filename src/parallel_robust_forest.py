...
################################################### PRIVATE FUNCTIONS ###################################################
    ...
    def __compute_attacks(self, X, attacks_filename):
        ...
        with open(attacks_filename, 'wb') as attacks_file:
            dill.dump(self.attacks, attacks_file,
...
class Constraint(object):
    ...
    def propagate_left(self, attacker, feature_id, feature_value, is_numerical):
        ...
        if is_numerical:
            ...
            attacks = [atk for atk in attacks if atk[0]
    ...
    def encode_for_optimizer(self, direction):
        ...
        if direction == 'L':
            if self.ineq == 0:  # encoding less than or equal to
                encoded_constraint['fun'] = lambda pred: - \
                    (pred[0] - self.y)**2 + (self.bound - self.y)**2
        ...
        if direction == 'R':
            if self.ineq == 0:  # encoding less than or equal to
                encoded_constraint['fun'] = lambda pred: - \
                    (pred[1] - self.y)**2 + (self.bound - self.y)**2
            else:  # encoding greater than or equal to
                encoded_constraint['fun'] = lambda pred: (
                    pred[1] - self.y)**2 - (self.bound - self.y)**2
        ...
        if direction == 'U':
            if self.ineq == 0:  # encoding less than or equal to
                encoded_constraint['fun'] = lambda pred: - \
                    np.max((pred - self.y)**2) + (self.bound - self.y)**2
            else:  # encoding greater than or equal to
                encoded_constraint['fun'] = lambda pred: np.min(
                    (pred - self.y)**2) - (self.bound - self.y)**2
...
class SplitOptimizer(object):
    ...
    def __optimize_sse_under_max_attack(self,
        ...
        if not res.success:
            self.logger.error(
                "!!!!!!!!!!!!!!!!!! Solver Error: {} !!!!!!!!!!!!!!!!!!".format(res.message))
            return None
    ...
    def optimize_gain(self, X, y, rows, numerical_idx, feature_blacklist, n_sample_features, replace_features,
                      attacker, costs,
                      constraints, current_score, current_prediction_score):
        """
        ...
        actual_features = [f_id for f_id in range(
            np.size(X, 1)) if f_id not in feature_blacklist]
        # 2. randomly sample a subset of n features out of the actual features
        self.logger.info("Randomly sample (without replacement) {} out of the total number of features ({})".
                         format(n_sample_features, len(actual_features)))
        actual_features = sorted(np.random.choice(actual_features, size=min(
            n_sample_features, len(actual_features)), replace=replace_features))
        feature_map = dict(
            (f_id, sorted(list(set(X[:, f_id])))) for f_id in actual_features)  # range(np.size(X, 1)) if f_id not in feature_blacklist)
        ...
        for feature_id, feature_values in feature_map.items():
            ...
            for feature_value_idx, feature_value in enumerate(feature_values):
                ...
                if optimizer_res:
                    ...
                    self.logger.debug("Optimization result\nFeature ID: {}; Threshold:{}\n[L={:.3f}; R={:.3f}; SSE_UMA={:.3f}]"
                                      .format(feature_id, feature_value, y_pred_left, y_pred_right, sse_uma))

                    n_left = len(split_left)
                    n_right = len(split_right)

                    self.logger.debug(
                        "Number of instances ending up in the left branch: {}".format(n_left))
                    self.logger.debug(
                        "Number of instances ending up in the right branch: {}".format(n_right))
                    ...
                    if gain > best_gain:
                        ...
                        best_split_feature_value = feature_value  # save the best splitting value
                        ...
                        if feature_value_idx < len(feature_values) - 1:
                            ...
                            next_best_split_feature_value = feature_values[feature_value_idx + 1]
                        else:
                        ...
                        if numerical_idx[best_split_feature_id]:
                            self.logger.debug("Best splitting strategy so far: [feature id {}; feature value = {:.5f}; next feature value = {:.5f}; actual split value = {:.5f}]"
                                              .format(best_split_feature_id,
                                                      best_split_feature_value,
                                                      next_best_split_feature_value,
                                                      best_split_feature_value)
                                              )
                        else:
                            self.logger.debug("Best splitting strategy so far: [feature id {}; feature value = {}; next feature value = {}; actual split value = {}]"
                                              .format(best_split_feature_id,
                                                      best_split_feature_value,
                                                      next_best_split_feature_value,
                                                      best_split_feature_value)
        ...
        return (best_gain, best_split_left_id,
                best_split_right_id, best_split_feature_id,
                best_split_feature_value, next_best_split_feature_value,
                best_pred_left, best_pred_right, best_sse_uma,
...
class RobustDecisionTree(BaseEstimator, ClassifierMixin):
    ...
    def __fit(self, X_train, y_train, rows, attacker, costs, node_prediction,
        ...
        self.logger.info("Current node's prediction: {}".format(
            current_prediction))
        ...
        if X.shape[0] < self.min_instances_per_node:
            self.logger.info("Number of instances ended up in the current node {} are less than the minimum number of instances at each node of this tree {}".format(
        ...
        if best_gain > 0.0001:  # TO FIX!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            if self.numerical_idx[best_split_feature_id]:
                ...
                self.logger.info("The best gain obtained is: {:.5f} using [feature id {}; feature value = {:.5f}; next feature value = {:.5f}; actual split value = {:.5f}]"
                                 .format(best_gain,
                                         best_split_feature_id,
                                         best_split_feature_value,
                                         next_best_split_feature_value,
            ...
            # the feature is categorical
                ...
                node.best_split_feature_value = best_split_feature_value
                self.logger.info("The best gain obtained is: {:.5f} using [feature id {}; feature value = {}; next feature value = {}; actual split value = {}]"
                                 .format(best_gain,
                                         best_split_feature_id,
                                         best_split_feature_value,
                                         next_best_split_feature_value,
            ...
            self.logger.info("Number of left rows: {}".format(
                len(best_split_left_id)))

            # save the current feature blacklist
            updated_feature_blacklist = feature_blacklist

            if self.is_affine:
                self.logger.debug("Update feature blacklist removing the best split feature id found [{}]"
                                  .format(best_split_feature_id))
...
# This class represents an _ensemble_ of robust decision trees, i.e., a **Robust Forest**, which can be used either for classification or regression.
# It exposes a quite familiar API which basically consists of two main methods:
#
# -  <code>**fit(X, y)**</code>
# -  <code>**predict(X)**</code>
#
# The former is invoked to train (i.e., learn) a robust forest by fitting it on a dataset of input observations (<code>X</code>) and associated labels/target values (<code>y</code>). To do so, this class makes use of the <code>RobustDecisionTree</code> class above.<br />
# When creating a <code>RobustForest</code> reference, few **hyperparameters** can be also specified. At least, one should choose the number of robust decision trees which the forest will be made of (i.e., the number of base estimators), and the number of features randomly sampled at each level of the tree-building stage.

class RobustForest(object):
    """
    This class implements a Robust Forest, i.e., an ensemble of Robust Decision Trees.
    Inspired by sklearn API, it exposes two main methods:
    - fit(X, y)
    - predict(X)
    The former is used at training time for learning the forest;
    ...
    def __init__(self,
        ...
        self.attacker = self.base_estimator.attacker
        self.split_optimizer = self.base_estimator.split_optimizer
        if n_estimators < 1:
            n_estimators = 100
        self.n_estimators = n_estimators
        self.max_depth = self.base_estimator.max_depth
        self.min_instances_per_node = self.base_estimator.min_instances_per_node
        self.max_samples = self.base_estimator.max_samples
        self.max_features = self.base_estimator.max_features
        self.feature_blacklist = self.base_estimator.feature_blacklist
        self.is_affine = self.base_estimator.is_affine
        # if this ensemble just contains a single tree
        if self.n_estimators == 1:
            # base estimator will not bootstrap samples
            self.base_estimator.bootstrap_samples = False

        self.estimators = []  # list of base estimators
        self.is_trained = False

        self.logger = logging.getLogger(__name__)

        self.logger.info("***** Robust Forest successfully created *****")
        self.logger.info("*\tForest ID: {}".format(self.forest_id))
        self.logger.info(
            "*\tSplitting criterion: {}".format(self.split_optimizer.split_function_name))
        self.logger.info("*\tBase estimator: {}".format(self.base_estimator))
        self.logger.info("*\tN. of estimators: {}".format(self.n_estimators))
        self.logger.info("*\tMax depth: {}".format(self.max_depth))
        self.logger.info(
            "*\tMin instances per tree node: {}".format(self.min_instances_per_node))
        self.logger.info(
        ...
        self.logger.info(
            "*\tMax features: {:.1f}%".format(self.max_features * 100))
        self.logger.info(
            "*\tFeature blacklist: {}".format(set(list(self.feature_blacklist.values()))))
        self.logger.info("*\tAffinity: {}".format(self.is_affine))
        self.logger.info(
            "*****************************************************")
    ...
    def fit(self, X, y=None, dump_filename='./robust_forest', dump_n_trees=10):
        ...
        # # prepare the list of the same input matrix X (i.e., one for each estimator)
        # l_X = [X for i in range(self.n_estimators)]
        # # prepare the list of the same input label y (i.e., one for each estimator)
        # l_y = [y for i in range(self.n_estimators)]

        # pool = ProcessingPool(processes=mp.cpu_count())
        # self.estimators = pool.map(
        #     self.base_estimator.zip_fit, zip(l_X, l_y))
